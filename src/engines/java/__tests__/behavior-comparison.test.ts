 ...
 * expectation is wrong; reconcile against `java -e` if available.
 *
...
function expectCleanStack(source: string, expected: string) {
  ...
  const r = execJava(source);
  const last = r.snapshots[r.snapshots.length - 1];
  // After execution, no block-scope (for/while) frame should remain.
...
describe('Java behavior comparison', () => {
  ...
  // ── Inline array literals (REGRESSION) ──
  // The parser produces `arrayCreationWithInitializerSuffix` (no "Expression"
  // infix) for `new int[]{...}`, but the interpreter used to only check
  // `arrayCreationExpressionWithInitializerSuffix`. The result was a `null`
  // array silently substituting for the inline literal.
  ...
  // The for-loop pushes a block-scope frame for `i`. An early return must
  // unwind it (otherwise the frame leaks into every subsequent snapshot).
  ...
  it('bubble sort with array swap', () => expectStdout(wrap(`
    int[] a = {5, 2, 8, 1, 9, 3};
  ...
  `.replace('java.util.ArrayList', 'ArrayList')), '3\n20'));
