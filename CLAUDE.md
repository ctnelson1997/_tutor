...
| Java behavior comparison | `src/engines/java/__tests__/behavior-comparison.test.ts` | End-to-end snippets with hand-computed expected stdout (matching real javac semantics). Includes regression coverage for short-circuit `&&` / `||`, inline `new int[]{...}` literals, return-inside-for stack cleanup, StringBuilder / ArrayList / HashMap method dispatch (including chained calls like `sb.append("a").append("b")`), and static inner class instance methods. Add new cases here when fixing engine bugs found via real-world Java code. |
